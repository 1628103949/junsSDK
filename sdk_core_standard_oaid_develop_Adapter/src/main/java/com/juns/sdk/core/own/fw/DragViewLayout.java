...
public class DragViewLayout extends FrameLayout {
    ...
    private void startHideAnim(){
        ValueAnimator valueAnimator;
        if (floatLayoutParams.x < mScreenWidth / 2) {
            valueAnimator = ValueAnimator.ofInt(floatLayoutParams.x, -mWidth / 2);
        } else {
            valueAnimator = ValueAnimator.ofInt(floatLayoutParams.x, mScreenWidth - mWidth + mWidth/2);
        }
        valueAnimator.setDuration(200);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
    ...
    public void wmAddView(View view){
        WindowManager.LayoutParams viewLayoutParams = new WindowManager.LayoutParams(WindowManager.LayoutParams.WRAP_CONTENT,
                WindowManager.LayoutParams.WRAP_CONTENT, WindowManager.LayoutParams.FIRST_SUB_WINDOW,
                WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE | WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL
                        | WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,
                PixelFormat.RGBA_8888);
        ...
        if(isRightFloat()){
            ...
            viewLayoutParams.x = SizeUtils.dp2px(55);
        }else {
            viewLayoutParams.gravity = Gravity.LEFT | Gravity.TOP;
            viewLayoutParams.x = SizeUtils.dp2px(0);
        }
        int y = (int) (yInScreen - yInView);
        ...
        mWindowManager.addView(view, viewLayoutParams);
    }
