...
import com.juns.sdk.core.sdk.OaidHelper;
...
//    }
    ...
    private void dealDataFlow(JunSResponse tnResponse) {
        try {
            if (TextUtils.isEmpty(tnResponse.data) || tnResponse.data.equals("[]")) {
                Bus.getDefault().post(EvInit.getFail(JunSConstants.Status.PARSE_ERR, "MActive parser data error.1"));
            } else {
            ...
            e.printStackTrace();
            Bus.getDefault().post(EvInit.getFail(JunSConstants.Status.PARSE_ERR, "MActive parser data error.2"));
        }
    }
    ...
    private void dealConfig(JSONObject dataJson) {
        try {
            ...
            e.printStackTrace();
            Bus.getDefault().post(EvInit.getFail(JunSConstants.Status.PARSE_ERR, "MActive parser data error.3"));
        }
    }
    //初始化心跳
    ...
    private void dealUpdate(final JSONObject dataJson) {
        try {
            ...
            e.printStackTrace();
            Bus.getDefault().post(EvInit.getFail(JunSConstants.Status.PARSE_ERR, "MActive parser data error.4"));
        }
    }
