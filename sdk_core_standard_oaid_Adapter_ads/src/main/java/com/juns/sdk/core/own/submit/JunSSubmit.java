...
public class JunSSubmit {
    ...
    public void doSubmit(HashMap<String, String> eventInfo) {
        ...
        if(eventInfo.get(JunSConstants.SUBMIT_TYPE).equals(JunSConstants.SUBMIT_TYPE_ENTER)){
            x.http().post(eventSubmitParam, new Callback.CommonCallback<JunSResponse>() {
                @Override
                public void onSuccess(final JunSResponse result) {
                    //Log.e("guoinfo", "eventSubmitParam: "+result.toString());
                    String data = result.data;
                    try {
                        //Log.e("guoinfo", "onSuccess: "+result.toString());
                        ...
                        if (dataJson.has("is_shop")) {
                            SDKData.setIsShop(dataJson.toString());
                        }else {
                            JSONObject dataJson2 = new JSONObject("{\"is_shop\":0,\"is_red\":0}");
                            SDKData.setIsShop(dataJson2.toString());
                        }

                    } catch (JSONException e) {
                        JSONObject dataJson2 = null;
                        try {
                            dataJson2 = new JSONObject("{\"is_shop\":0,\"is_red\":0}");
                        } catch (JSONException jsonException) {
                            jsonException.printStackTrace();
                        }
                        assert dataJson2 != null;
                        SDKData.setIsShop(dataJson2.toString());
                        e.printStackTrace();
                    }
                    if (result.state == 0) {
