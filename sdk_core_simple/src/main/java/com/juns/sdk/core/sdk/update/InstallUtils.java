...
import com.juns.sdk.framework.utils.ShellUtils;
...
public final class InstallUtils {
     ...
     * apk安装的请求码
     */
    public static final int REQUEST_CODE_INSTALL_APP = 1993;
     ...
     * 是否支持静默安装【默认是true】
     */
    private static boolean sSupportSilentInstall = false;

    private InstallUtils() {
        throw new UnsupportedOperationException("Do not need instantiate!");
    }

    private static boolean isSupportSilentInstall() {
    ...
    public static void setSupportSilentInstall(boolean supportSilentInstall) {
        InstallUtils.sSupportSilentInstall = supportSilentInstall;
    }
    ...
    public static boolean install(Context context, String filePath) {
        if (InstallUtils.isSystemApplication(context)
    ...
    public static Intent getInstallAppIntent(Context context, File appFile) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                Uri fileUri = FileProvider.getUriForFile(context, context.getApplicationContext().getPackageName() + ".JunSFileProvider", appFile);
