...
//    ExecutorService tpe = Executors.newFixedThreadPool(1);
     ...
     */
    public Bitmap getBitmap(Activity act, final String path, int width, int height, final ILoadImageCallback callback, Object... objects) {
        Bitmap bitmap = getBitmapFromCache(path, width, height);
        //如果能够从缓存中获取符合要求的图片，则直接回调
        if (bitmap != null) {
            Log.e("zhao", "get bitmap from cache");
        } else {
            decodeBitmapFromPath(act, path, width, height, callback, objects);
        }
        return bitmap;
    }
     ...
     * 通过path获取图片bitmap
     */
    private void decodeBitmapFromPath(final Activity act, final String path, final int width, final int height, final ILoadImageCallback callback, final Object... objects) throws OutOfMemoryError {
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (act.getApplication().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (act.getApplication().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
                    ...
                    File file = new File(CommonUtil.getDataPath(act));
                    if (!file.exists()) {
                        file.mkdirs();
                    }
                    //临时文件的文件名
                    String tempPath = CommonUtil.getDataPath(act) + hash + ".temp";
